# SATOMI Supabase SQL Draft

Last updated: 2026-06-04

Status: draft only. Do not execute blindly in production before review.

```sql
-- SATOMI Supabase schema draft
-- Phase 12: Supabase backend planning
-- This SQL is intended for Supabase Postgres after manual review.
...
create index if not exists nudges_user_created_idx
  on public.nudges (user_id, created_at desc);

create index if not exists nudges_user_action_status_idx
  on public.nudges (user_id, action_status);
...
create index if not exists notification_sources_user_package_idx
  on public.notification_sources (user_id, package_name);
...
create index if not exists notification_candidates_user_status_created_idx
...
- This draft uses lowercase stable values for status/type fields.
- `transactions.amount` is positive; `transactions.type` decides income or
  expense.
- Composite foreign keys include `user_id` to prevent cross-user links to
  another user's pocket, transaction, or goal.
- `profiles_delete_own` is included for symmetry, but production account
  deletion may prefer a controlled server-side flow.
- This draft does not create seed data.
- This draft does not create storage buckets.
- This draft does not add service-role logic.
